...
export default function CreateSubmissionForm({ tracks }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input
        className="mt-1 mb-4 block w-full rounded-md bg-ocean-400 p-2 text-lg shadow-lg focus:border-teal-600 focus:outline-none focus:ring focus:ring-cyan-600"
        type="text"
        id="name"
        name="name"
      ...
      <textarea
        className="text-m mt-1 mb-4 block rounded-lg bg-ocean-400 p-2 shadow-lg focus:border-teal-600 focus:outline-none focus:ring focus:ring-cyan-600"
        id="description"
        name="description"
        rows={10}
        cols={55}
      ...
      <div id="tracks" className="space-y-2">
        {tracks.map((track, index) => (
          <Switch
            className={({ checked }) =>
              `${checked ? "bg-cyan-600" : "bg-ocean-400"}
                    block text-left ${
